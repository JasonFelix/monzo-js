...
module.exports = class Transactions extends Endpoint {
  ...
  async find(transactionId) {
    try {
      return new Transaction(this._client, (await this.get(`${transactionId}?expand[]=merchant&account_id=${this._accountId}`)).data.transaction);
    } catch (err) {
  ...
  async query(config = undefined) {
    try {
      const res = (await this.get(`?expand[]=merchant&account_id=${this._accountId}&${this._buildConfig(config)}`)).data.transactions;
